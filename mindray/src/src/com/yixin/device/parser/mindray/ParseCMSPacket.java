...
import com.yixin.device.parser.mindray.CMSPacket.Field;
import com.yixin.device.parser.mindray.ECGFilteringWay.FilteringWayData;
import com.yixin.device.parser.mindray.ECGGain.ECGWaveGain;
import com.yixin.device.parser.mindray.ECGGain.ECGWaveGainCoefficient;
import com.yixin.device.parser.mindray.ECGWaveformName.WaveName;
...
public class ParseCMSPacket implements PackageParseIntf
{
	...
	public ECGWaveformName parseWaveFormName(CMSPacket pack)
	{
		...
		if (waveName.getWaveNameList() != null) { return waveName; }
		return null;
	}
	...
	public ECGFilteringWay parseECGFilteringWay(CMSPacket pack)
	{
		...
		if (ECGdata.getListData().size() <= 0) { return null; }
		return ECGdata;
	}
	...
	public ECGGain parseECGGain(CMSPacket pack)
	{
		...
		if (ecgGain.getECGGainLength() <= 0) { return null; }
		return ecgGain;
	}
	...
	public List<ECGWaveData> parseECGwavedata(CMSPacket pack)
	{
		...
		if (wdList.size() != 0) { return wdList; }
		return null;
	}
	...
	{
		if (data.length < startLength + 7) { return ""; }
