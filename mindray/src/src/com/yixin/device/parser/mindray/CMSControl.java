...
public class CMSControl implements IBluetoothParser
{
	 ...
	 * 
	 */
	public int parser(byte[] dataIn, int start)
	{
		//是否为全新的数据包
			...
			if (nLen > 0)
			{
				...
				if (packet.isPkgCheckOk())
				{
					...
					if (this.isSeqencePacket(packet))
					{
						...
						{
							//处理该数据包流
							processPacketStream(packetStreamLast);
							//清空数据包流
							packetStreamLast = null;
						}
						if (packetStreamLast == null)
	 ...
	 * 
	 */
	public void processPacketStream(CMSPacketStream packetStream)
	{
		//如果数据包流传输不正确，则不处理该数据包流
		if (!packetStream.isPacketStreamCorrect()) { return; }
