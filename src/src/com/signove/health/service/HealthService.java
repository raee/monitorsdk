...
import com.yixin.monitors.sdk.bluetooth.BluetoothManager;
...
public class HealthService extends Service {
	...
	private HealthServiceAPI.Stub apiEndpoint = new HealthServiceAPI.Stub() {
		...
		@Override
		public void ConfigurePassive(HealthAgentAPI agt, int[] specs)
	...
	private Handler mIncomingHandler = new Handler() {
		 ...
		 * @param msg
		 */
		private void sendConnectedBroadcast(Message msg) {
			BluetoothDevice mDevice;
			if (!mIsSendConnectedBroadcast) {

				BluetoothAdapter bluetoothAdapter = BluetoothAdapter
						.getDefaultAdapter();
				for (BluetoothDevice device : bluetoothAdapter
						.getBondedDevices()) {
					if (OmronBluetoothConnection.DEVICE_NAME.equals(device
							.getName())) {
						// 发送蓝牙广播，告知已经完成连接！
						SendBoradcastManager.sendDeviceConnected(
	...
	public synchronized int create_timer(int milisseconds, int context) {
		...
		Log.i(TAG, "Creating timer " + timer_id + " timeout " + milisseconds
				+ "ms");
	...
	private void sendMessageWithData(int what, Object[] pair) {
		...
		try {
			mHealthService.send(Message.obtain(null,
					BluetoothHDPService.MSG_SEND_DATA, pair));
		} catch (RemoteException e) {
			Log.i(TAG, "Unable to reach service.");
			e.printStackTrace();
		}
